...
const BOT_CODES = ["NUMBER_BOT_001", "NUMBER_BOT_002", "NUMBER_BOT_003", "NUMBER_BOT_004"];

function ensureBotAccounts() {
  const insert = db.prepare(
    "INSERT OR IGNORE INTO game_bot_accounts (game_id, code, display_name) VALUES (?, ?, ?)"
  );
  BOT_CODES.forEach((code, i) => insert.run(GAME_ID, code, `Player${2200 + i}`));
}
ensureBotAccounts();
...
const BET_TYPES = ["odd", "even", "small", "big", ...Array.from({ length: 10 }, (_, n) => `single:${n}`)];

// Same isolation guarantee as every other game's bot module: bots never see
// the pre-committed result, never touch game_wallet_ledger or a real
// wallet_balance (no user_id), and this is the only place a bot bet is ever
// created.
function scheduleBotBets(roundId, cfg, onBotBet) {
  ...
  for (const bot of participants) {
    ...
    setTimeout(() => {
      ...
      const payoutMultiplier = betType.startsWith("single:") ? cfg.singlePayout : cfg.categoryPayout;
