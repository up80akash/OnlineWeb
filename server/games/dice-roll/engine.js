...
const state = {
  roundId: null,
  roundNumber: 0,
  status: "idle", // idle | betting | rolling | completed
  serverSeed: null,
  serverSeedHash: null,
  clientSeed: null,
  nonce: 0,
...
function isValidBetType(betType) {
  if (["low", "high", "odd", "even"].includes(betType)) return true;
  const m = /^exact:([1-6])$/.exec(betType);
  return !!m;
}

function payoutFor(betType, cfg) {
  return betType.startsWith("exact:") ? cfg.exactPayout : cfg.categoryPayout;
}

function isWinningBet(betType, result) {
  if (betType === "low") return result <= 3;
  if (betType === "high") return result >= 4;
...
function publicState() {
  ...
  return {
    ...
    minBet: cfg.minBet,
    maxBet: cfg.maxBet,
    exactPayout: cfg.exactPayout,
    categoryPayout: cfg.categoryPayout,
    result: state.status === "completed" || state.status === "rolling" ? state.result : null,
...
function runRound() {
  ...
  const result = pf.deriveInt(serverSeed, clientSeed, nonce, 0, 6) + 1;
...
function closeBetting(roundId) {
  ...
  db.prepare("UPDATE game_rounds SET status = 'rolling' WHERE id = ?").run(roundId);
  ws.broadcast(NAMESPACE, "BETTING_CLOSED", { roundId });
  ws.broadcast(NAMESPACE, "RESULT_GENERATED", { roundId });
  ws.broadcast(NAMESPACE, "DICE_ROLLING", { roundId });
...
function revealResult(roundId) {
  ...
  db.prepare("UPDATE game_rounds SET status = 'resolved', resolved_at = ?, server_seed = ?, result = ? WHERE id = ?").run(
    resolvedAt,
    state.serverSeed,
    JSON.stringify({ face: state.result }),
    roundId
  );

  ws.broadcast(NAMESPACE, "DICE_RESULT", { roundId, face: state.result });
  ws.broadcast(NAMESPACE, "RESULT_REVEALED", { roundId, face: state.result, serverSeed: state.serverSeed, serverSeedHash: state.serverSeedHash, clientSeed: state.clientSeed, nonce: state.nonce });

  settleRound(roundId, state.result, cfg);

  ws.broadcast(NAMESPACE, "ROUND_COMPLETED", { roundId, face: state.result });
    ...
    if (won) {
      ...
      if (bet.user_id) {
        wallet.credit({
          ...
          userId: bet.user_id,
          amount: prize,
          idempotencyKey: `dice_roll:win:${bet.id}`,
  ...
  for (const bet of orphaned) {
    ...
    if (bet.user_id) {
      wallet.refundBet({
        ...
        idempotencyKey: `dice_roll:refund-restart:${bet.id}`,
      });
    }
  }
  runRound();
...
function placeBet({ userId, betType, betAmount, idempotencyKey }) {
  ...
  const run = db.transaction(() => {
    const debit = wallet.debitForBet({
      ...
      userId,
      amount: betAmount,
      idempotencyKey: idempotencyKey ? `${idempotencyKey}:debit` : `dice_roll:bet:${userId}:${roundId}:${betType}:${Date.now()}`,
    });
    const info = db
      .prepare(
...
function verify(serverSeed, clientSeed, nonce) {
  const face = pf.deriveInt(serverSeed, clientSeed, nonce, 0, 6) + 1;
  return {
    face,
    serverSeedHash: pf.hashServerSeed(serverSeed),
    formula: "face = 1 + floor((HMAC_SHA256(server_seed, client_seed + ':' + nonce + ':0') first 13 hex chars / 2^52) * 6)",
  };
}
...
function recentRounds(limit = 20) {
  return db
    ...
    .all(GAME_ID, limit)
    .map((r) => ({ roundNumber: r.roundNumber, face: JSON.parse(r.result).face, resolvedAt: r.resolvedAt }));
}
